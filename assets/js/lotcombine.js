...
$(document).ready(function() {
    ...
    $('#employeeForm').submit(function(e) {
        ...
        $.ajax({
            url: "../lotcombine/get_employee_details",
            type: "GET",
            dataType: "json",
            ...
            success: function(response) {
                ...
                if (response && response.data) {
                    ...
                    var serial1 = employee.serial1;
                    var id = employee.id;
                    var disposition = employee.disposition;
...
$('#save_appenddata_btn').click(function(e) {
   ...
   var referenceModel = firstRow.find('td:nth-child(4)').text();
   var referenceCavity = firstRow.find('td:nth-child(5)').text();
   ...
   tableRows.each(function(index) {
       var model = $(this).find('td:nth-child(4)').text();
       var cavity = $(this).find('td:nth-child(5)').text();
       var quantity = parseInt($(this).find('td:nth-child(6)').text(), 10); // Parse as integer

       // Add quantity to total
       totalQuantity += quantity;

       if (model !== referenceModel || cavity !== referenceCavity) {
           isValid = false;
           errorMsg = 'All rows must have the same Model and Cavity as the first row!';
   ...
   if (totalQuantity > 32) {
       Swal.fire({
           ...
           text: 'The total quantity of all rows exceeds 32!' + totalQuantity
       });
  ...
  // Implement the save logic here
    ...
    $('#items_table tbody tr').each(function() {
        var serial1 = $(this).find('td:first').text();
        ...
        requestData.push({ serial1: serial1, id: id, disposition: disposition, model: model, cavity: cavity, quantity: quantity});
       
       
    });
    // Send the data to the server for insertion
    $.ajax({
        url: "../lotcombine/add_request", // URL of your server-side endpoint
        type: "POST",
        dataType: "json",
