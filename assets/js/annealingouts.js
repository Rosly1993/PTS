...
var url = '../annealingouts/get_castinglocks' +
...
console.log(id); // Check if IDs are captured correctly
   ...
   if(id.length == 0){
       ...
       $.ajax({
           url: '../annealingouts/delete_records',
           method: 'POST',
           data: {
               id: id,
...
$('#checkAll').on('click', function () {
    ...
    var hasNG = $('.row-checkbox').filter(function () {
        return $(this).closest('tr').find('td:eq(12)').text().trim() === 'NG';
    }).length > 0;
    ...
    $('.row-checkbox').each(function () {
        ...
        if (row.find('td:eq(12)').text().trim() !== 'NG' && parseFloat(row.find('td:eq(9)').text().trim()) >= parseFloat(row.find('td:eq(10)').text().trim())) {
...
$('#mold_no').empty().append('<option value="">Select Tooling No</option>');
