...
$(document).ready(function () {
    var table = $('#items_table').DataTable({
        "ajax": {
            "url": "../machiningsplits/get_prresults",
            "data": function (d) {
        ...
        "columns": [
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    return '<a class="me-2 btn-sm editbtn rounded-0 py-0 edit_data" href="javascript:void(0)" data-id="' + (row.id) + '" data-url="../machiningsplits/get_prresult_details/"><i class="ni ni-settings"></i></a>';
                }
            }
        ],
        "responsive": true,
...
$('#addItemForm').on('submit', function (e) {
    ...
    $('#items_table tbody').on('click', '.edit_data', function () {
        ...
        function edit_data(id) {
            ...
            $.ajax({
                url: "../machiningsplits/get_prresult_details/" + id,
                type: "GET",
                dataType: "json",
...
var new_serial = $('#new_serial').val().trim();

if (quantityLost < 1) {
// Handle invalid input, perhaps show an alert
Swal.fire({
icon: 'error',
title: 'Invalid Split Quantity Input',
...
if (quantity < 2) {
    ...
    Swal.fire({
    icon: 'warning',
    title: 'Quanity is not Splittable',
    text: 'Quantity is Less Than 4'
    });
    return; // Exit the function to prevent further execution
    }
...
if (new_serial ==='') {
    ...
    icon: 'error',
    title: 'Invalid Input',
    text: 'New Serial is empty.'
    });
    return; // Exit the function to prevent further execution
    }

    if (new_serial.length !== 12) {
        // Handle invalid input, perhaps show an alert
        Swal.fire({
        icon: 'error',
        title: 'Invalid Input',
...
$.ajax({
url: "../machiningsplits/update_prresults",
type: "POST",
data: formData,
dataType: "json",
