...
class Splitlottaggings extends CI_Controller {
    public function __construct() {
        parent::__construct();
        $this->load->library('session');
        $this->load->helper('url'); // Load URL Helper
        $this->load->model('splitlottaggings_model');
    ...
    public function index(){
        ...
        if($this->session->userdata('user')){
            ...
            $data['prresults'] = $this->splitlottaggings_model->get_prresults();
            ...
            if($area_id =='19' || $area_id =='22'){
                ...
                $this->load->view('Splitlottaggings/index', $data);
    ...
    public function get_prresults() {
        ...
        $data = $this->splitlottaggings_model->get_prresults($line);
        echo json_encode(['data' => $data]); // Wrap the data in 'data' key
    }
    public function get_prresult_details($id) {
        $prresult_details = $this->splitlottaggings_model->get_prresult_details($id);
    ...
    public function update_prresults() {
        ...
        $data = array(
            ...
            'sample_by' => $userdata,
            'sample_date' => $updated_at,
            'split_lot_details' => $category.'--'.$remarks,
