...
class Molddetails extends CI_Controller {
    ...
    public function add_molddetails() {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $cavity_no = $this->input->post('cavity_no');
            ...
            if ($existing_mold) {
                ...
                $response['message'] = 'Tooling Number already exists!';
            }else{

           
                // If the item doesn't exist, add it to the database
                $this->molddetails_model->add_molddetails($model, $mold_no, $cavity_no, $cavity1, $cavity2, $cavity3, $cavity4, $created_by);
        
                // Return a success response
                $response['status'] = 'success';
                $response['message'] = 'Tooling Number added successfully!';
            }
        ...
        public function update_molddetails() {
            ...
            if ($existing_combination && $existing_combination['id'] != $id) {
                ...
                echo json_encode(array('status' => 'duplicate', 'message' => 'Combination of Model and Tooling Number already exists.'));
                return;
            }
