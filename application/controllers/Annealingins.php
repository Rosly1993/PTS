...
class Annealingins extends CI_Controller {
    public function __construct() {
        parent::__construct();
        $this->load->library('session');
        $this->load->helper('url'); // Load URL Helper
        $this->load->model('lines_model');
        $this->load->model('annealingins_model');
    ...
    public function index(){
        ...
        if($this->session->userdata('user')){
            ...
            $data['lines'] = $this->lines_model->get_lines_by_area('4'); // Pass '2' as area_id
            ...
            if($area_id =='19' || $area_id =='4'){
                ...
                $this->load->view('Annealingins/index', $data);
    ...
    public function get_quantity() {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($serial1 !== false && $serial1 !== '') {
                ...
                $quantity = $this->annealingins_model->get_quantity($serial1);
    ...
    public function add_lot() {
        if ($this->input->is_ajax_request()) {
            ...
            $result = $this->annealingins_model->add_lot($serial1, $line);
    ...
    public function get_models() {
        $data = $this->annealingins_model->get_models();
