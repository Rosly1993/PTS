...
class Annealingouts extends CI_Controller {
    public function __construct() {
        parent::__construct();
        $this->load->library('session');
        $this->load->helper('url'); // Load URL Helper
        $this->load->model('annealingouts_model');
    ...
    public function index(){
        ...
        if($this->session->userdata('user')){
            ...
            $data['castinglocks'] = $this->annealingouts_model->get_castinglocks();
            $data['models'] = $this->models_model->get_models(); // Fetch models data
            $data['molddetails'] = $this->molddetails_model->get_molddetails();

            $data['lines'] = $this->lines_model->get_lines_by_area('4');
            // Load the areas view with data
           
            $user = $this->session->userdata('user');
            extract($user);
           
            if($area_id =='19' || $area_id =='4'){

                $this->load->view('Annealingouts/index', $data);
    ...
    public function get_castinglocks() {
        ...
        $data = $this->annealingouts_model->get_castinglocks(); // No need to pass $line as an argument
    ...
    public function delete_records() {
        ...
        foreach ($ids as $id) {
            ...
            $history_data = array(
                'annealing_date_out' => $locked_date,
                'annealing_pic_out' => $fullname
                // Add more fields as necessary
            );
            ...
            $next_id_result = $this->db->get_where('set_process', array('area_id' => '4','model' => $result->model))->row();
