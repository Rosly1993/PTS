...
class Machiningins extends CI_Controller {
    public function __construct() {
        parent::__construct();
        $this->load->library('session');
        $this->load->helper('url'); // Load URL Helper
        $this->load->model('lines_model');
        $this->load->model('machiningins_model');
    ...
    public function index(){
        ...
        if($this->session->userdata('user')){
            ...
            if($area_id =='19' || $area_id =='5'){
                ...
                $this->load->view('Machiningins/index', $data);
    ...
    public function get_quantity() {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($serial1 !== false && $serial1 !== '') {
                ...
                $quantity = $this->machiningins_model->get_quantity($serial1);
    ...
    public function add_lot() {
        if ($this->input->is_ajax_request()) {
            ...
            $result = $this->machiningins_model->add_lot($serial1, $line);
    ...
    public function get_models() {
        $data = $this->machiningins_model->get_models();
