<?php
class Annealingins_model extends CI_Model {
    ...
    public function add_lot($serial1, $line) {
        ...
        $this->db->select('pr_result.disposition,pr_result.serial1, pr_result.unique1, pr_result.input_by, pr_result.area_id, pr_result.id, set_areas.area');
        ...
        if ($query->num_rows() > 0) {
            $row = $query->row();
            if ($row->area == 'Annealing' && $row->input_by === null && $row->disposition =='OK') {
                ...
                $data_history = array(
                    'annealing_date_in' => $date_add,
                    'annealing_pic_in' => $fullname,
                    'annealing_oven' => $line,
                    // add other fields you want to insert
                );
                ...
                //for the alert ang ferson
                  if ($row->area == 'Annealing' && $row->input_by === null && $row->disposition !='OK') {
                    return "This Serial is Active on Annealing but need QA-Shotblast Judgement";
                  }else{
