...
class Printing_model extends CI_Model {
	...
	public function get_areas() {
        ...
        $this->db->select('
        pr_result.id,
        pr_result.serial1 as plate_serial,
        pr_result.model,
        pr_result.input_by,
        pr_result.input_date,
       sum(quantity - quantity_lost)  as actual,
        pr_result.cavity,
        pr_result.mfi_serial
    ');
    
    $this->db->from('pr_result');
    $this->db->where('disposition', 'Accepted');
    ...
    public function get_area_details_by_control_number($control_number) {
        ...
        $query = $this->db->get_where('pr_result', ['mfi_serial' => $control_number, 'disposition' => 'Accepted']);
        
        // Return the result as an array
        return $query->result_array();
    }

    public function update_status($id, $status, $remarks, $date_add, $fullname,$area_status,$lot_status) {
