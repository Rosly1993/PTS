<?php
class Ovenouts_model extends CI_Model {
	...
	public function get_castinglocks() {
        ...
        $this->db->select('a.*, b.disposition_ipqc, c.disposition');
        $this->db->from('(SELECT 
        pr_result.id, 
        pr_result.serial1,
        pr_result.area_id,
    ...
    WHERE 
        ...
        $this->db->join('(SELECT serial1, unique1, MAX(id) AS max_id, disposition_ipqc FROM pr_result WHERE area_id = "5" GROUP BY serial1, unique1) b', 'a.serial1 = b.serial1 AND a.unique1 = b.unique1', 'left');  
        $this->db->join('(SELECT serial1, unique1, MAX(id) AS max_id, disposition FROM pr_result WHERE area_id = "1" GROUP BY serial1, unique1) c', 'a.serial1 = c.serial1 AND a.unique1 = c.unique1', 'left');    
