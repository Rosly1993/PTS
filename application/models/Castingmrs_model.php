<?php
class Castingmrs_model extends CI_Model {
    ...
    $this->db->from('(SELECT
        ...
        pr_result.unique1
        FROM pr_result
        WHERE pr_result.disposition="NG" AND pr_result.area_id="1"
        GROUP BY pr_result.serial1) c');
    $this->db->join('(SELECT
        pr_result.serial1,
        pr_result.unique1,
        ...
        JOIN (SELECT serial1, MAX(id) AS max_id FROM pr_result GROUP BY serial1) a
