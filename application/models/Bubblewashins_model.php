<?php
class Bubblewashins_model extends CI_Model {
    ...
    public function add_lot($serial1, $line) {
        ...
        if ($query->num_rows() > 0) {
            $row = $query->row();
            if ($row->area == 'BUBBLE WASH' && $row->input_by === null) {
                ...
                $data_history = array(
                    'bubblewashin_date' => $date_add,
                    'bubblewashin_pic' => $fullname,
                    'bubblewashin_line' => $line,
                    // add other fields you want to insert
                );
