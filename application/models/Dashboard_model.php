...
class Dashboard_model extends CI_Model {
	...
	public function get_prresults($serial = null) {
        ...
        $subquery = '(SELECT pr_result.lot_status,pr_result.id,pr_result.serial1,pr_result.unique1,pr_result.model, pr_result.cavity,pr_result.area_status,pr_result.debplan, (pr_result.quantity - pr_result.quantity_lost) as newquantity, set_areas.area 
                     FROM `pr_result` 
                     left join set_areas on pr_result.area_id = set_areas.id
                    
                     WHERE serial1 = \'' . $serial . '\' 
                     ORDER BY pr_result.id DESC) a';
    
        // Using the subquery as the main table and grouping by serial1 and unique1
        $this->db->from($subquery);
        $this->db->group_by('a.serial1');
        $this->db->group_by('a.unique1');
